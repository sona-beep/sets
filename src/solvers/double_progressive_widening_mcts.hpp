...
class DPW_MCTS : public Solver {
    ...
    public: 
        ...
        void set_param(int N, int max_depth, double wct, double c, bool export_topology, 
                ...
                std::string heuristic_mode, std::string tree_exploration, bool downsample_traj_on, 
                double alpha, int init_num_children, int num_timesteps_hold, bool verbose) {
            ...
            m_alpha = alpha;
            m_init_num_children = init_num_children;
        ...
        double rollout(RNG& rng, int rollout_count, Node* node, int depth, Trajectory &traj_from_root, MDP *mdp, Tree & tree) {
            ...
            while (true) {
                ...
                double widening = std::pow( double(node->num_visits), m_alpha);
                if (node->children.size() < widening) {
                    // expand node 
                ...
                if (node->children[argmax] == nullptr) {
                    ...
                    node->children[argmax]->children.resize(m_init_num_children);
                    ...
                    Eigen::VectorXd u = sample_vec_from_cube(mdp->U(), rng);
        ...
        SolverResult solve(Eigen::VectorXd state, int timestep, MDP *mdp, RNG& rng) override {
            ...
            root->children.resize(m_init_num_children);
