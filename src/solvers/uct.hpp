...
class UCT : public Solver {
    ...
    public: 
        ...
        void set_param(int N, int max_depth, double wct, double c, bool export_topology, bool export_states, bool export_trajs, 
                bool export_cbdsbds, bool export_tree_statistics, std::string heuristic_mode, std::string tree_exploration, bool downsample_trajs, bool verbose) {
            m_N = N; 
        ...
        double rollout(RNG& rng, int rollout_count, Node* node, int depth, Trajectory &traj_from_root, MDP *mdp, Tree & tree) {
            ...
            int next_cbd_idx;
            if (m_export_cbdsbds){
                next_cbd_idx = tree.cbds.size();
                tree.cbds.resize(next_cbd_idx+1);
                tree.sbds.resize(next_cbd_idx+1);
            }
            ...
            while (true) {
                ...
                if (node->children[argmax] == nullptr) {
                    ...
                    mdp->expand_ii(rng, argmax, node->traj.xs.back(), 
                        node->traj.us.back(), node->children[argmax]->traj, 
                        node->cbd, 
                        node->children[argmax]->sbd, 
                        node->children[argmax]->wctd);
                ...
                if (m_export_cbdsbds) {
                    tree.cbds[next_cbd_idx] = node->cbd;
                    tree.sbds[next_cbd_idx] = node->children[argmax]->sbd;
                }
